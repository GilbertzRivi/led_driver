    ...
    for (uint8_t i = 0; i < server.args(); i++)
      {
        if (server.argName(i) == "r"){
          ...
          turnOffTime = epochTime + ((server.arg(i).toInt()*60)-1);
          Serial.println(server.argName(i) + server.arg(i));
        }
      }    
  }
...
String mainPage(){
  ...
  ptr += "        </div>\n";
  ptr += "        <div class=\"flexrow\">\n";
  ptr += "            <div class=\"value\" class=\"value\">Auto Turn Off: </div>\n";
  ptr += "            <input name=\"time\" type=\"number\" max=\"60\" id=\"time\" value=\"time\">\n";
  ptr += "            <br>\n";
  ptr += "            </div>\n";
